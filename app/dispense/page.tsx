  ...
  const handlePrintLabel = async () => {
    ...
    for (const it of items) {
      const label = [
        ...
        `วิธีใช้: ${[it.route, `ครั้งละ ${it.dose_qty ?? 1} ${it.dose_unit || ''}`.trim(), it.frequency, it.meal_relation].filter(Boolean).join(' ')}`,
        it.meal_sessions ? `ช่วงเวลา: ${it.meal_sessions.split(',').map((s: string) => s.trim()).filter(Boolean).join(' ')}` : '',
